...
gas/
  ...
  src/
    Config.js             固定値・対応表（機種/カラーの表示名、SKU管理番号ルール、在庫数の固定値）
...
`docs/reference/column-mapping.csv` の区分のうち、SKU行の主要列を実装済み：

- システム連携用SKU番号（区分A：商品マスターの商品コードをそのまま）
- SKU管理番号（区分B：機種依存ルール、既存値保持オプションあり）
